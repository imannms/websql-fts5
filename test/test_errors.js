exports.test = function(sql, assert) {
  ...
  // Execute some sql
  var res = db.execute("CREATE TABLE test (a INTEGER PRIMARY KEY, b, c, d, e);");

  assert.throws(function(){
    db.execute("I ain't be no valid sql ...");
  },
                /syntax error/,
  ...
  stmt.run([2]);
  assert.deepEqual(db.execute("SELECT a,b FROM test WHERE a=2"),
