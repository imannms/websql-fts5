exports.test = function(SQL, assert){
  ...
  db.execute("CREATE TABLE test (data); INSERT INTO test VALUES ('Hello World');");
  ...
  var result = db.execute("SELECT TestFunction(data) FROM test;");
  ...
  db.execute("CREATE TABLE test2 (int1, int2); INSERT INTO test2 VALUES (456, 789);");
  ...
  result = db.execute("SELECT TestAdd(int1, int2) FROM test2;");
  ...
  db.execute("CREATE TABLE test3 (data); INSERT INTO test3 VALUES (x'6100ff'), (x'ffffff00ffff');");
  ...
  result = db.execute("SELECT TestZeroByteIndex(data) FROM test3;");
  ...
  result = db.execute("SELECT addOne(1);");
  ...
  function canHandle(testData)
  {
    ...
    result = db.execute("SELECT CheckTestValue("+sql_value+")==12345"); 
    ...
    result = db.execute("SELECT GetTestValue()");
    ...
    if(sql_value!=="null")
    {
      result = db.execute("SELECT identityFunction("+sql_value+")="+sql_value); 
    }else
    {
      result = db.execute("SELECT identityFunction("+sql_value+") is null"); 
    }
    if(result[0]["values"][0][0]!=1)
    {
      if(verbose)
  ...
  assert.throws(function(){db.execute("SELECT throwFunction()");},/internal exception/, "Can handle internal exceptions");
  
  db.create_function("customeObjectFunction", function () { return {test:123};} );    
  assert.throws(function(){db.execute("SELECT customeObjectFunction()");},/Wrong API use/, "Reports wrong API use");
